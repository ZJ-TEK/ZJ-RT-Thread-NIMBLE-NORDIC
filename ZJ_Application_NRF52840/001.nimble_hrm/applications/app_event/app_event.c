...
static int app_event_init(void)
{
    ...
    if(app_event_mutex.owner == NULL)
    {
        ...
        while(1)
        {
            if(app_event_timer.timeout_func)
            {
                break;
            }
        }
...
static void app_timeout_process_thread(void *args)
{
    ...
    { 
        ...
        RT_ASSERT(rt_mutex_take(&app_event_mutex, 1000) == RT_EOK);
        ...
        for (i = 0; i < MAX_DELAY_EVENT; i++)
        {
            if (event_delay_treat.event[i])
            {
                RT_ASSERT(event_delay_treat.delay[i] > 0);
        ...
        RT_ASSERT(rt_mutex_release(&app_event_mutex) == RT_EOK);
